...
function pickSpecs(product) {
  ...
  return {
    // All 186 products set their own specs.MOQ, so this fallback should never
    // fire; it exists only so a newly added product without the field cannot
    // silently publish a number nobody chose. Kept as a range rather than a
    // confident single figure for that reason.
    moq: s.MOQ || '200–500 pcs depending on the design',
