...
import { buildRichDescription, getProductContentLabels } from '@/lib/product-content';
...
// image lives inside the body copy (not only the top gallery). Text generated
// from the product's own data, so every PDP renders genuinely different
// sentences — different materials, MOQs, use cases, etc. Localised per
...
@media (max-width: 600px) { .pdp-rich { padding: 48px 18px; } }
`;

export default function ProductRichBlock({ product, locale = 'en' }) {
  const blocks = buildRichDescription(product, locale);
  ...
  return (
    <section className="pdp-rich" aria-labelledby="pdp-rich-title">
      ...
      <div className="pdp-rich-inner">
        <div className="pdp-rich-eyebrow">{labels.richEyebrow}</div>
        <h2 className="pdp-rich-title" id="pdp-rich-title">{labels.richTitle(product.name)}</h2>
        {blocks.map((b, i) =>
          b.type === 'paragraph' ? (
